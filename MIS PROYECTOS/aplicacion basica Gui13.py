...
class SimpleApp:
    def __init__(self, root):
        ...
        """
        self.root = root
        self.root.title("Ventana Lams")  # Establece el título de la ventana

        self.root.geometry("300x324")  # Establece el tamaño de la ventana
        self.root.resizable(True, False)  # Hace que la ventana no sea redimensionable en altura
        self.root.config(bg="blue")  # Establece el color de fondo de la ventana

        try:
            self.root.iconbitmap("lams.ico")  # Establece el icono de la ventana
        except tk.TclError:
            print("Error: lams.ico not found or not a valid icon file.  Ignoring icon.")
