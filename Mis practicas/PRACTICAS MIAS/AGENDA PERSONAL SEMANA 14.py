...
class AgendaApp:
    ...
    def create_widgets(self):
        ...
        frame_entrada: ttk.Frame = ttk.Frame(self.root, padding="10")
    ...
    def open_calendar(self):
        ...
        top.title("Seleccionar Fecha")  # Agregado un título a la ventana emergente
        top.geometry("300x200")  # Establecido un tamaño mínimo para la ventana
    ...
    def agregar_evento(self):
        ...
        # Validación adicional para la hora ingresada
        try:
            hora_partes = hora.split(":")
            hora_valida = int(hora_partes[0]) in range(24) and int(hora_partes[1]) in range(60)
        except ValueError:
            hora_valida = False

        if not hora_valida:
            messagebox.showerror("Error", "Hora inválida. Por favor, ingrese una hora en formato HH:MM.")
            return
    ...
    def eliminar_evento(self):
        ...
        if not messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este evento?"):
            return
...
if __name__ == "__main__":
    try:
        ...
        messagebox.showerror("Error", f"Ocurrió un error al ejecutar la aplicación: {e}")